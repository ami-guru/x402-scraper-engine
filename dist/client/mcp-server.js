...
class X402ScraperMcpServer {
            ...
            tools: [
                {
                    ...
                    description: 'Scrapes any public webpage and returns clean, sanitized, token-efficient Markdown for LLM ingestion. Automatically handles HTTP 402 microtransactions (0.005 USDC on Base) if required.',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            url: {
                ...
                },
                {
                    ...
                    description: 'Scrapes target webpage and executes Edge LLM (Llama 3) context synthesis to extract executive summary, key takeaways, and structured entities. Automatically handles HTTP 402 microtransactions (0.025 USDC on Base).',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            url: {
                ...
                },
                {
                    ...
                    description: 'Performs security, credibility, and phishing risk analysis for any website or smart contract landing page. Returns credibility score (0-100) and risk signals. Automatically handles HTTP 402 microtransactions (0.080 USDC on Base).',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            url: {
                ...
                },
                {
                    ...
                    description: 'Performs deep web research across multiple sources, extracting clean Markdown summaries from top results. Automatically handles HTTP 402 microtransactions (0.05 USDC on Base).',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            query: {
                ...
                {
                    name: 'twitter_search',
                    description: 'Searches public tweets, cashtags ($BTC, $BASE), and sentiment across Twitter/X without the $100/mo API fee. Automatically handles HTTP 402 microtransactions (0.05 USDC on Base).',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            query: {
                ...
                },
                {
                    ...
                    description: 'Extracts public Twitter/X profile bio and recent tweets for any username without the $100/mo API fee. Automatically handles HTTP 402 microtransactions (0.03 USDC on Base).',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            username: {
     ...
     */
    async executeScrapeWithAutoPayment(targetUrl) {
        ...
        if (initialRes.status === 402) {
            const paymentData = await initialRes.json().catch(() => ({}));
            const recipient = initialRes.headers.get('X-Payment-To') ||
            ...
            const tokenAddress = initialRes.headers.get('X-Payment-Asset-Address') ||
                paymentData.payment?.contractAddress ||
                DEFAULT_USDC_BASE;
            if (!recipient || recipient === '0x0000000000000000000000000000000000000000') {
                throw new Error('HTTP 402 received, but treasury recipient address is not properly configured on worker.');
            }
            // Check agent private key
            if (!AGENT_PRIVATE_KEY || AGENT_PRIVATE_KEY.startsWith('0x000000000000')) {
                throw new Error(`HTTP 402 Payment Required: Service charges ${amount} USDC on Base.\n` +
                    `Recipient: ${recipient}\n` +
            ...
            if (!paidRes.ok) {
                const errorBody = await paidRes.json().catch(() => ({}));
                throw new Error(`Payment submitted (${txHash}), but worker returned HTTP ${paidRes.status}: ${errorBody.details || errorBody.message || errorBody.error || paidRes.statusText}`);
            }
            const finalData = await paidRes.json();
     ...
     */
    async executeDigestWithAutoPayment(targetUrl, focus) {
        ...
        if (initialRes.status === 402) {
            ...
            if (!AGENT_PRIVATE_KEY || AGENT_PRIVATE_KEY.startsWith('0x000000000000')) {
                throw new Error(`HTTP 402 Payment Required: Edge LLM digest charges ${amount} USDC on Base.\n` +
                    `Recipient: ${recipient}\n` +
     ...
     */
    async executeAuditWithAutoPayment(targetUrl, contractAddress) {
        ...
        if (initialRes.status === 402) {
            ...
            if (!AGENT_PRIVATE_KEY || AGENT_PRIVATE_KEY.startsWith('0x000000000000')) {
                throw new Error(`HTTP 402 Payment Required: Security audit charges ${amount} USDC on Base.\n` +
                    `Recipient: ${recipient}\n` +
                    `Please configure a funded AGENT_PRIVATE_KEY in .env to proceed.`);
            }
            const txHash = await this.sendUsdcPayment(recipient, amount, tokenAddress);
     ...
     */
    async executeSearchWithAutoPayment(query, limit) {
        ...
        if (initialRes.status === 402) {
            ...
            if (!AGENT_PRIVATE_KEY || AGENT_PRIVATE_KEY.startsWith('0x000000000000')) {
                throw new Error(`HTTP 402 Payment Required: Deep search charges ${amount} USDC on Base.\n` +
                    `Recipient: ${recipient}\n` +
                    `Please configure a funded AGENT_PRIVATE_KEY in .env to proceed.`);
            }
     ...
     */
    async executeTwitterSearchWithAutoPayment(query, limit) {
        ...
        if (initialRes.status === 402) {
            ...
            if (!AGENT_PRIVATE_KEY || AGENT_PRIVATE_KEY.startsWith('0x000000000000')) {
                throw new Error(`HTTP 402 Payment Required: Twitter search charges ${amount} USDC on Base.\n` +
                    `Recipient: ${recipient}\n` +
                    `Please configure a funded AGENT_PRIVATE_KEY in .env to proceed.`);
            }
            const txHash = await this.sendUsdcPayment(recipient, amount, tokenAddress);
     ...
     */
    async executeTwitterProfileWithAutoPayment(username) {
        ...
        if (initialRes.status === 402) {
            ...
            if (!AGENT_PRIVATE_KEY || AGENT_PRIVATE_KEY.startsWith('0x000000000000')) {
                throw new Error(`HTTP 402 Payment Required: Twitter profile lookup charges ${amount} USDC on Base.\n` +
                    `Recipient: ${recipient}\n` +
                    `Please configure a funded AGENT_PRIVATE_KEY in .env to proceed.`);
            }
            const txHash = await this.sendUsdcPayment(recipient, amount, tokenAddress);
     ...
     */
    async sendUsdcPayment(recipient, amountUsdc, tokenAddress) {
...
function formatMarkdownOutput(data) {
    let header = '';
    if (data.title) {
        header += `# ${data.title}\n\n`;
    }
...
function formatSearchResultsOutput(data) {
